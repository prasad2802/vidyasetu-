...
Simple UI + CSV/Firestore logging + Report download
Exports `demo` (for server.py to mount)
"""
...
# Logging / files
LOG_PATH     = os.getenv("LOG_PATH", "progress.csv")   # e.g. /data/progress.csv on Railway PV
REPORT_DIR   = os.getenv("REPORT_DIR", "reports")      # where final report .md files will be saved
...
def explain_with_groq(prompt: str) -> str:
    """Return a string; never raise so the UI keeps loading even if key is wrong."""
    ...
    try_order = ([_LAST_WORKING_MODEL] if _LAST_WORKING_MODEL else []) + [
        m for m in GROQ_FALLBACKS if m != _LAST_WORKING_MODEL
    ]
...
def _choices_with_distractors(correct, spread=5, count=4):
    ...
    out = list(opts); random.shuffle(out); return out
...
def q_fractions(diff: str) -> Tuple[str,str,List[str]]:
    ...
    opts = {ans}
    if den!=1: opts.add(f"{num+1}/{den}"); opts.add(f"{max(1,num)}/{max(1,den-1)}"); opts.add(f"{num+2}/{den}")
    else: opts |= {str(num+1), str(num+2), str(num-1)}
...
def q_decimals(diff: str):
    ...
    x = round(random.uniform(1,limit),1); y = round(random.uniform(1,max(5,limit/5)),1)
    op = random.choice(["+","−","×"])
...
def q_algebra(diff: str):
    ...
    c = a*x + b; q = f"Solve for x: {a}x + ({b}) = {c}"
...
def q_geometry(diff: str):
    ...
    if kind=="area_rect": l,w = random.randint(3,scale), random.randint(3,scale); ans=l*w; q=f"Area of rectangle (l={l} cm, w={w} cm)?"
    elif kind=="perim_rect": l,w = random.randint(3,scale), random.randint(3,scale); ans=2*(l+w); q=f"Perimeter of rectangle (l={l} cm, w={w} cm)?"
    elif kind=="area_circle": r = random.randint(2,max(5,scale//2)); ans=round(3.14*r*r,2); q=f"Area of circle (r={r} cm). Use π≈3.14."
    else: b,h = random.randint(4,scale), random.randint(4,scale); ans=0.5*b*h; q=f"Area of triangle (base={b} cm, height={h} cm)?"
...
def q_sit(diff: str):
    ...
    si = p*r*t/100; q = f"Simple Interest on ₹{p} at {r}% p.a. for {t} years?"
    return q, str(int(si)), _choices_with_distractors(int(si), spread=100)

def q_sdt(diff: str):
    s = random.randint(20, {"Easy":80,"Medium":120,"Hard":180}[diff])
    t = random.randint(1,6); d = s*t; kind = random.choice(["distance","time","speed"])
...
def _get_fs_client():
    global _FS_CLIENT
    if _FS_CLIENT is not None: return _FS_CLIENT
    if not (USE_FIRESTORE and firestore): return None
    try:
...
def _log_firestore(row: dict):
    ...
    try:
        client.collection(FS_COLLECTION).add(row); return True
    except Exception:
        return False

def log_attempt(row: dict):
    ok = False
    if USE_FIRESTORE and firestore: ok = _log_firestore(row)
    if not ok: _log_csv(row)

# --------- Helpers for UI downloads ----------
def _refresh_csv():
    path = os.getenv("LOG_PATH", "progress.csv")
    file_out = path if (os.path.exists(path) and os.path.isfile(path)) else None
    return path, file_out

# ------------- Build the simple UI (export `demo`) -------------
with gr.Blocks(title="Vidya Setu — Tutor + Adaptive Exam") as demo:
    gr.Markdown("# Vidya Setu — Tutor & Adaptive Exam")
    ...
    with gr.Tab("Adaptive Exam"):
        ...
        student_id = gr.Textbox(label="Student ID (for progress)", placeholder="e.g., 2025A001")
        ...
        def start_exam(student, topics):
            ...
            csv_path, csv_dl = _refresh_csv()
            return status, q, gr.update(choices=choices, value=None), curr, prog, score, st, "", None, csv_path, csv_dl

        def check_answer(choice, st):
            if not st or st.get("answer") is None:
                csv_path, csv_dl = _refresh_csv()
                return "Click **Start Exam** first.", st, "", None, csv_path, csv_dl
            ...
            if st["correct_in_topic"] >= TOPIC_TARGET_CORRECT:
                ...
                if st["topic_idx"] >= len(topics):
                    ...
                    return ("🎉 Exam finished!", "", gr.update(choices=[], value=None),
                            "", "", f"**Overall correct:** {st['score_total']}", st, report_md, report_path, csv_path, csv_dl)
                ...
                return ("➡️ Topic complete. Moving on.", q, gr.update(choices=choices, value=None),
                        curr, prog, score, st, "", None, csv_path, csv_dl)

            # still same topic → new question
            q, ans, choices, diff = gen_question(topic, st["diff_idx"])
            st["answer"] = ans
            st["last_question"] = q
            curr = f"**Topic:** {topic}  |  **Difficulty:** {diff}"
        ...
        start_btn.click(
            ...
            [status, question_md, options, curr_topic, progress, score_box, exam_state, report, report_file, csv_path_show, csv_file]
        )
        check_btn.click(
            check_answer, [options, exam_state],
            [feedback, exam_state, report, report_file, csv_path_show, csv_file]
        )
        next_btn.click(
            next_step, [exam_state],
            [feedback, question_md, options, curr_topic, progress, score_box, exam_state, report, report_file, csv_path_show, csv_file]
        )
        download_btn.click(_refresh_csv, [], [csv_path_show, csv_file])
